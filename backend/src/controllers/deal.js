...
const createDeal = async (req, res) => {
  try {
    ...
    if (maxDeals !== -1 && activeDealCount >= maxDeals) {
      return res.status(400).json({
        success: false,
        error: {
          code: 'DEAL_LIMIT_REACHED',
          message: `Your ${restaurant.subscriptionTier} subscription allows maximum ${maxDeals} active deals. You currently have ${activeDealCount} active deals.`
        }
      });
    }
