...
/**
 * Generate verification code for deal redemption
 * POST /api/v1/redemptions/generate-code
 */
const generateVerificationCode = async (req, res) => {
  try {
    ...
    // If pending redemption exists and is less than 24 hours old, return it
    if (redemption) {
        ...
        return res.status(200).json({
          ...
          data: {
            redemption: {
              ...
              code: redemption.verificationCode,
...
/**
 * Verify code and complete redemption
 * POST /api/v1/redemptions/verify-code
 */
const verifyAndCompleteRedemption = async (req, res) => {
  try {
    const { verificationCode, orderAmount } = req.body;
    ...
    if (req.user.role !== 'ADMIN') {
      const restaurant = await prisma.restaurant.findFirst({
        where: {
          id: redemption.deal.restaurantId,
          ownerId: merchantId
        }
      });
      ...
      if (!restaurant) {
        return res.status(403).json({
          ...
          error: {
            code: 'UNAUTHORIZED',
            message: 'You do not have permission to redeem this deal'
          }
        });
      }
    }
    ...
    let finalAmount = parseFloat(orderAmount) || 0;

    switch (deal.discountType) {
      case 'PERCENTAGE':
        discountAmount = (finalAmount * deal.discountValue) / 100;
        break;
      case 'FIXED':
        discountAmount = Math.min(deal.discountValue, finalAmount);
        break;
      case 'BOGO':
        discountAmount = finalAmount * 0.5;
        break;
    }
