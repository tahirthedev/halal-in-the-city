...
function Users() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const transformedUsers = response.data.map(user => ({
          ...
          email: user.email,
          role: user.role === 'RESTAURANT_OWNER' ? 'restaurant_owner' : 'customer',
          status: user.status ? user.status.toLowerCase() : 'active',
          joinedDate: user.createdAt,
          dealsAvailed: user._count?.redemptions || 0,
          lastActive: user.updatedAt || user.createdAt,
          phone: user.phone || 'N/A',
          restaurantName: user.restaurants?.[0]?.name || null,
        }))
  ...
  const handleStatusChange = async (newStatus) => {
    if (editingUser) {
      try {
        await apiService.toggleUserStatus(editingUser.id)
